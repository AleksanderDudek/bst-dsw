...
//brakuje: rotacja w lewo (odwrtone dzialanie do rotacji w prawo, denominacja na poziom o 1 nizszy),
//         MAX glebokosc BST
...
int ObliczWysokosc(Wezel* wskaznik) //internet, tablica sciezek od wszystkich lisci
{
	...
	int lewy = ObliczWysokosc(wskaznik->lewy);
	int prawy = ObliczWysokosc(wskaznik->prawy);
...
void RotujPrawo(Wezel*& korzen, Wezel*& dziadek, Wezel*& rodzic, Wezel*& dziecko) //wyklad
{
	if (dziadek != NULL)
	{
		if (dziadek->prawy == rodzic)
		{
	...
	Wezel* kontener = dziecko->prawy;
	dziecko->prawy = rodzic;
	rodzic->lewy = kontener;
}

int ZrobListeWezlow(Wezel* &korzen) //wyklad
{

	Wezel* dziadek = NULL, *kontener = korzen, *kontenerDwa = NULL;

	//rotacje w prawo w celu uzyskania listy jednokierunkowej
	while (kontener != NULL)
	{
		if ((kontener->lewy != NULL))
		{
			kontenerDwa = kontener->lewy;
			RotujPrawo(korzen, dziadek, kontener, kontener->lewy);
			kontener = kontenerDwa;
		}
		else
		{
			dziadek = kontener;
			kontener = kontener->prawy;
		}
	}


	//obliczanie ilosci elementow
	Wezel* wskaznik = korzen;
	int ilosc = 0;
...
// rodzic promowanego elementu stake sie jego lewym dzieckiem
// jesli rodzic ma potomka to lewy potomek promowanego elementu szuka
// sobie nowego miejsca

void RotujLewoDwa (Wezel*& korzen, Wezel*& dziadek, Wezel*& rodzic, Wezel*& dziecko)
{
	if ( dziadek != NULL)
	{
		dziadek->prawy = dziecko;
	...
	rodzic->prawy = dziecko->lewy;
	dziecko->lewy = rodzic;
}

void RotujLewo(Wezel*& korzen, Wezel*& dziadek, Wezel*& rodzic, Wezel*& dziecko)
{
	if (dziadek != NULL)
	{
		//to wiadomo, jesli prawe dziecko dziadka jest rodzicem
		//to dziadek adoptuje dziecko(swojego wnuka) (rodzica, czyli swojego dziacka)
		if (dziadek->prawy == rodzic)
		{
			dziadek->prawy = dziecko;
		}
	...
	{  //to wiadomo, jesli jest gdzies w glebi promowany, to 
		//z algorytmu wynika, ze jako 2 element na prawej linii bedzie 1 (czyli rootem)
		korzen = dziecko;
	}

	//teraz jesli dziecko sie unosi to przekazuje swojego lewego potomka
	Wezel* kontener = dziecko->lewy;
	dziecko->lewy = rodzic;
	rodzic->prawy = kontener;

}

void ZrobPerfekcyjnaListe(Wezel* &korzen, int n)// wyklad
{
	Wezel* dziadek = NULL;
	Wezel* kontener = korzen;
	Wezel* kontenerDwa = NULL;
	...
	//w tej petli sie wypierdalam na RotujLewo
	for (int i = 0; i < (n - m); i++)
	{
		kontenerDwa = kontener->prawy;
		if (kontenerDwa != NULL)
		{
			RotujLewo(korzen, dziadek, kontener, kontener->prawy);
			dziadek = kontenerDwa;
			kontener = kontenerDwa->prawy;
		}
	}
	while (m > 1)
	{
...
void WykonajDSW(Wezel* &korzen)
{
	//faza I - rotacja w prawo kolejnych wezlow idac w prawa strone (liczac od korzenia) lewych potomkow 
	// while (current->left != NULL ) rotate_right

	int n;

	n = ZrobListeWezlow(korzen);

	ZrobPreorder(korzen);
		//faza II - rotacja w lewo okreslonych wezlow
		//podfaza sklada sie na rotacje co drugiego elementu (korzen = 1 czyli nie podlega)
		//do ilosci N - m, gdzie N - glebokosc, m uzyskuje z kodu:
		// int count_m (int N)
		// m = 1; 
		// while (m <=N) m= 2* m +1;
		// m=m/2; 
...
int main()
{
	...
	WstawWiele(korzen, X1);

	cout << "Wstawione" << endl;

	height =  ObliczWysokosc(korzen);
	...
	korzen = NULL;
	//delete(korzen);
	//tutaj robi sie error, usuwanie chyba zle zadzialalo

	WstawWiele(korzen, X2);
